...
export const SetupFormInsert=async(model?:SetupForm,menuCode?:string):Promise<HttpResponse<SetupForm>>=>{
...
export const SetupFormDelete=async(model?:SetupForm,menuCode?:string):Promise<HttpResponse<SetupForm>>=>{
...
export const SetupFormGetOne=async(code?:string,menuCode?:string):Promise<HttpResponse<SetupForm>>=>{
   
   let info=  await get_type_by_menu_code(menuCode)
   let res= await QuantomGET<SetupForm>(INVENTORY_SETUP_FORM_GET_ONE+`?Type=${info?.Type}&Code=${code}`,true);
   return res;
}


export const SetupFromGetAll=async(menuCode?:string,searchText?:string):Promise<SetupForm[]>=>{

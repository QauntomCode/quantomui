...
import {
  CustomerPaymentReceiptDtoModel,
  VmCustomerPaymentModel,
} from "../model/CustomerPaymentReceiptModel";
...
export const CustomerPaymentReceiptInsertMethod = async (
  data?: VmCustomerPaymentModel
): Promise<HttpResponse<VmCustomerPaymentModel>> => {
...
export const CustomerPaymentReceiptDeleteMethod = async (
  data?: VmCustomerPaymentModel
): Promise<HttpResponse<VmCustomerPaymentModel>> => {
  let res = await QuantomPOST<VmCustomerPaymentModel>(
    CUSTOMER_RECEIPT_DELETE_URL,
    true,
    data
  );
  return res;
...
export const CustomerPaymentReceiptGetOneMethod = async (code?: string) => {
  let res = await QuantomGET<VmCustomerPaymentModel>(
    CUSTOMER_RECEIPT_GET_ONE_URL + "?code=" + code,
    true
  );
  return res;
};

export const CustomerPaymentReceiptGetAll = async (
  fromDate?: Date,
  toDate?: Date,
  locId?: string,
  search?: string
): Promise<CustomerPaymentReceiptDtoModel[]> => {
  let data = {
    FromDate: fromDate,
    ToDate: toDate,
    LocId: locId,
  ...
  let res = await QuantomPOST<CustomerPaymentReceiptDtoModel[]>(
    CUSTOMER_RECEIPT_GET_ALL_URL,
    true,
    data
  );
  return res?.Response ?? [];

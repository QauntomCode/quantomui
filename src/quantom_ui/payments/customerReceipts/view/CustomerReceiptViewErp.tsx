...
export const CustomerReceiptViewErp=(props?:MenuComponentProps<VmCustomerPaymentModel>)=>{
    ...
    useEffect(()=>{
        if(fullState?.IsFirstUseEffectCall){
          setFormBasicKeys<VmCustomerPaymentModel>({
            ...
            baseProps:props??{},
            GetOneMethod:payload=>CustomerPaymentReceiptGetOneMethod(payload),
            SaveMethod:(payload,ctx)=>{
                let np={...payload};
                if(isNullOrEmpty(np?.master?.LocId) && ctx?.Location?.LocId){
                    np={...np,master:{...np.master,LocId:ctx?.Location?.LocId}}
                }
                if(!np?.master?.ReceiptDate){
                    np={...np,master:{...np.master,ReceiptDate:new Date()}}
                }
               return CustomerPaymentReceiptInsertMethod(np)},
            DeleteMethod:(payload,ctx)=>CustomerPaymentReceiptDeleteMethod(payload),

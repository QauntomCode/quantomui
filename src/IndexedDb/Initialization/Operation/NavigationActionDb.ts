...
import { NavigationActionInfo } from "../../../quantom_ui/Management/Common/NavigationModels/NavigationActionInfo";
...
export const DBGetSingleNavigationAction=async(menuCode:string):Promise<NavigationActionInfo[]>=>{
...
export const DBClearAllNavigationAction=async(nav?:NavigationActionInfo)=>{
  var db=await openDB();
  const tx= db.transaction(ACTION_NAVIGATION_STORE,"readwrite");
  var store= tx.objectStore(ACTION_NAVIGATION_STORE);
  store?.clear()
